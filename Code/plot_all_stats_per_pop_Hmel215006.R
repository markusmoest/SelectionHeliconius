...
names(ANNOT) <- c("contig", "HGC", "type", "con_start", "con_end", "dot", "str", "unk", "descr")
ANNOT2 <- read.table("gff/Hmel215006_cortex.gff",sep="\t")
...
# filesDiplo_Hmel215006 <- list.files(path='diploSHIC/', pattern='Hmel215006', full.names = T)
...
for(e in 1:length(pops)){
  # png(filename=paste("C:/Users/vanbe/Dropbox (Personal)/Manuscript_Selection/Figures/Suppl_stats_melp/Hmel215006_",names[e], ".png", sep=""), width = 1000, height = 1000)
  
  # print(pops[e])
  
  indxSF <- grep(popsSF[e], filesSF_Hmel215006)
  indxEHH <- grep(popsEHH[e], filesEHH_Hmel215006)
  indxStats <- grep(paste(pops[e],'.S.stats',sep=''), filesStats_Hmel215006)
  indxGenoPS <- grep(popsEHH[e], filesGenoPS_Hmel215006)
  # indxDiplo <- grep(pops[e], filesDiplo_Hmel215006)
  
  
  tableSF <- read.table(filesSF_Hmel215006[indxSF], h=T, stringsAsFactors=F)
  tableEHH <- read.table(filesEHH_Hmel215006[indxEHH], h=T, stringsAsFactors=F)
  tableStats <- read.csv(filesStats_Hmel215006[indxStats], h=T, stringsAsFactors=F)
  tableGenoPS <- read.csv(filesGenoPS_Hmel215006[indxGenoPS], h=T, stringsAsFactors=F)
  # tableDiplo <- read.table(filesDiplo_Hmel215006[indxDiplo], h=T, stringsAsFactors=F)
  ...
  text(1.8,0.2, substitute(paste(italic(nn)), list(nn=names[e])), cex=2)

  
  # Pi 
  
  plot(0,type='n',axes=FALSE,ann=FALSE,ylim=c(0,0.06), xlim = c(start,end))
  rect(1205164/1000000, 0, 1324501/1000000, 0.06, col = adjustcolor('gray', 0.5), border = NA)
  abline(h=0, col='lightgray') 
  ...
  rect(1205164/1000000, -3, 1324501/1000000, 3, col = adjustcolor('gray', 0.5), border = NA)
  abline(h=0, col='lightgray') 
  ...
  rect(1205164/1000000, 0, 1324501/1000000, 26, col = adjustcolor('gray', 0.5), border = NA)
  ...
  plot(0,type='n',axes=FALSE,ann=FALSE,ylim=c(0,700), xlim = c(start,end))
  rect(1205164/1000000, 0, 1324501/1000000, 700, col = adjustcolor('gray', 0.5), border = NA)

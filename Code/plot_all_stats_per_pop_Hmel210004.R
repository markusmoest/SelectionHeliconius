...
# ANNOT_Hmel218003 <- read.table("gff/Hmel218003.gff",sep="\t")
# names(ANNOT_Hmel218003) <- c("contig", "HGC", "type", "con_start", "con_end", "dot", "str", "unk", "descr")
# 
...
names(ANNOT) <- c("contig", "HGC", "type", "con_start", "con_end", "dot", "str", "unk", "descr")
ANNOT2 <- read.table("gff/Hmel210004_wntA_fixed.txt",sep="\t")
...
# filesDiplo_Hmel210004 <- list.files(path='diploSHIC/', pattern='Hmel210004', full.names = T)
...
for(e in 1:length(pops)){
  # png(filename=paste("C:/Users/vanbe/Dropbox (Personal)/Manuscript_Selection/Figures/Suppl_stats_melp/Hmel210004_",names[e], ".png", sep=""), width = 1000, height = 1000)
  
  # print(pops[e])
  
  indxSF <- grep(popsSF[e], filesSF_Hmel210004)
  indxEHH <- grep(popsEHH[e], filesEHH_Hmel210004)
  indxStats <- grep(paste(pops[e],'.S.stats',sep=''), filesStats_Hmel210004)
  indxGenoPS <- grep(popsEHH[e], filesGenoPS_Hmel210004)
  # indxDiplo <- grep(pops[e], filesDiplo_Hmel210004)
  
  
  tableSF <- read.table(filesSF_Hmel210004[indxSF], h=T, stringsAsFactors=F)
  tableEHH <- read.table(filesEHH_Hmel210004[indxEHH], h=T, stringsAsFactors=F)
  tableStats <- read.csv(filesStats_Hmel210004[indxStats], h=T, stringsAsFactors=F)
  tableGenoPS <- read.csv(filesGenoPS_Hmel210004[indxGenoPS], h=T, stringsAsFactors=F)
  # tableDiplo <- read.table(filesDiplo_Hmel210004[indxDiplo], h=T, stringsAsFactors=F)
  
  # EHHmask <- subset(tableGenoPS, tableGenoPS$genoPS < 0.1)
  # 
  # # removeL <- c()
  # # for(l in 1:nrow(tableEHH)){
  # #   for(e in 1:nrow(EHHmask)){
  # #     if(tableEHH$pos[l] >= EHHmask$start[e] & tableEHH$pos[l] >= EHHmask$start[e]){
  # #       removeL <- c(removeL, l)
  # #     }
  # #   }
  # # }
  # tableEHH <- tableEHH[-c(removeL),]
  ...
  text(2,0.2, substitute(paste(italic(nn)), list(nn=names[e])), cex=2)

  
  # Pi 
  
  plot(0,type='n',axes=FALSE,ann=FALSE,ylim=c(0,0.06), xlim = c(start,end))
  rect(1848666/1000000, 0, 1858224/1000000, 0.06, col = adjustcolor('gray', 0.5), border = NA)
  abline(h=0, col='lightgray') 
  
  # tableStats$lseff_egg <- as.numeric(tableStats$lseff_egg)
  ...
  rect(1848666/1000000, -3, 1858224/1000000, 3, col = adjustcolor('gray', 0.5), border = NA)
  abline(h=0, col='lightgray') 
  ...
  rect(1848666/1000000, 0, 1858224/1000000, 26, col = adjustcolor('gray', 0.5), border = NA)
  ...
  # for(e in 1:nrow(EHHmask)){
  #   rect(EHHmask$start/1000000, -1.5, EHHmask$end/1000000, 26, col = 'white', border = NA) # mask
  #   
  # }
  ...
  rect(1848666/1000000, 0, 1858224/1000000, 250, col = adjustcolor('gray', 0.5), border = NA)
  ...
  tableStats <- subset(tableStats, tableStats[,12] != "None")
  print(c(pops[e], mean(na.omit(tableStats[,6])), sd(na.omit(tableStats[,6])), mean(as.numeric(as.character(na.omit(tableStats[,12])))), sd(na.omit(tableStats[,12]))), quote = F)

...
ANNOT2 <- read.table("gff/Hmel201011_aristaless.gff",sep="\t")
...
# 
# ANNOT_Hmel210004 <- read.table("gff/Hmel210004.gff",sep="\t")
# names(ANNOT_Hmel210004) <- c("contig", "HGC", "type", "con_start", "con_end", "dot", "str", "unk", "descr")
...
for(e in 1:length(pops)){
  ...
  # png(filename=paste("C:/Users/vanbe/Dropbox (Personal)/Manuscript_Selection/Figures/Suppl_stats_melp/Hmel201011_",names[e], ".png", sep=""), width = 1000, height = 1000)
  
  # print(pops[e])
  
  indxSF <- grep(popsSF[e], filesSF_Hmel201011)
  indxEHH <- grep(popsEHH[e], filesEHH_Hmel201011)
  indxStats <- grep(paste(pops[e],'.S.stats',sep=''), filesStats_Hmel201011)
  indxGenoPS <- grep(popsEHH[e], filesGenoPS_Hmel201011)
  
  tableSF <- read.table(filesSF_Hmel201011[indxSF], h=T, stringsAsFactors=F)
  tableEHH <- read.table(filesEHH_Hmel201011[indxEHH], h=T, stringsAsFactors=F)
  tableStats <- read.csv(filesStats_Hmel201011[indxStats], h=T, stringsAsFactors=F)
  tableGenoPS <- read.csv(filesGenoPS_Hmel201011[indxGenoPS], h=T, stringsAsFactors=F)
  ...
  text(2.7,0.2, substitute(paste(italic(nn)), list(nn=names[e])), cex=2)

  
  # Pi 
  
  plot(0,type='n',axes=FALSE,ann=FALSE,ylim=c(0,0.06), xlim = c(start,end))
  rect(2508177/1000000, 0, 2515380/1000000, 0.06, col = adjustcolor('gray', 0.5), border = NA)
  abline(h=0, col='lightgray') 
  ...
  rect(2508177/1000000, -3, 2515380/1000000, 3, col = adjustcolor('gray', 0.5), border = NA)
  abline(h=0, col='lightgray') 
  ...
  rect(2508177/1000000, 0, 2515380/1000000, 26, col = adjustcolor('gray', 0.5), border = NA)
  ...
  plot(0,type='n',axes=FALSE,ann=FALSE,ylim=c(0,65), xlim = c(start,end))
  rect(2508177/1000000, 0, 2515380/1000000, 65, col = adjustcolor('gray', 0.5), border = NA)
  ...
  print(c(mean(na.omit(tableStats[,6])), sd(na.omit(tableStats[,6])), mean(na.omit(tableStats[,12])), sd(na.omit(tableStats[,12]))))
  # dev.off()
}
